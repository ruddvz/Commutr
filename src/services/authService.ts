...
export const authService = {
  async register(input: RegisterInput): Promise<AuthUser> {
    if (isDemoExperience()) return this.continueAsGuest(input.name)
  ...
  async login(input: LoginInput): Promise<AuthUser> {
    if (isDemoExperience()) return this.continueAsGuest()
  ...
  async fetchMe(): Promise<User> {
    if (isDemoExperience() || localStorage.getItem('commutr_token') === DEMO_TOKEN) {
      const demo = getDemoUser()
      return {
        id: demo.id,
        name: demo.name,
  ...
  isDemoGuest(): boolean {
    return localStorage.getItem('commutr_token') === DEMO_TOKEN || isDemoExperience()
  },
}
